...
$item replace entity @s container.$(value) with minecraft:stone[minecraft:custom_data={c:{cv:{config:1b,tool:1b}}}]
...
function c.cv:gui/configurer/utils/pages_internal/items/remove_item/pop_from_array_index with storage c:cv root.temp
